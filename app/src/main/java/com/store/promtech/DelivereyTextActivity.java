...
public class DelivereyTextActivity extends AppCompatActivity implements View.OnClickListener  {
    ...
    ImageView iv_cart;
    DeliveryText getDelivery;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_deliverey_text);
        mContext = this;
    ...
    private void setData() {
        tv_title.setText(getDelivery.getTermData().getContentTitle());
       // tv_desc.setText(Html.fromHtml(getDelivery.getTermData().getContent()));
        tv_desc.setText(getDelivery.getTermData().getContent());
    }
    ...
    private void getDeliveryTextData() {
        ...
        Call<DeliveryText> call = redditAPI.GetDeliveryText();
        call.enqueue(new Callback<DeliveryText>() {

            @Override
            public void onResponse(Call<DeliveryText> call, retrofit2.Response<DeliveryText> response) {
                Log.d("String", "" + response);
                if (response.isSuccessful()) {
                    getDelivery=response.body();
                    if(getDelivery.getTermData().getAck()==1) {
                        setData();
                    }
                }
            }
            ...
            @Override
            public void onFailure(Call<DeliveryText> call, Throwable t) {
                pDialog.dismiss();
            }
        });
    }
