...
public class ChangePasswordActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    public void onClick(View v) {
        if (v == btn_forgot) {
            if (et_oldpassword.getText().toString().isEmpty()) {
                Utility.showToastShort(mContext, "Please Enter Your Old Password");
                return;
            }

            if (et_newpass.getText().toString().isEmpty()) {
                Utility.showToastShort(mContext, "Please Enter Your New Password");
                return;
            }
            if (etconfirmpass.getText().toString().isEmpty()) {
                Utility.showToastShort(mContext, "Please Enter Your New Confirm Password");
                return;
            }

            if (!etconfirmpass.getText().toString().equalsIgnoreCase(et_newpass.getText().toString().trim())) {
                Utility.showToastShort(mContext, "New Password & Confirm Password doesn't matches");
                return;
            }
    ...
    private void verifyUser() {
        ...
        Call<BaseResponse> call = redditAPI.ChangePassword(user_oldpass, user_newpass, user_newpass, Preferences.get_userId(mContext));
